...
/**
 * ONNX-backed frame state classifier.
 */
public class OnnxFrameStateClassifier implements FrameStateClassifier {
    private static final FrameState[] CLASSES = new FrameState[]{
            FrameState.GOOD, FrameState.DARK, FrameState.BAD, FrameState.OFF_TARGET
    };
    ...
    public OnnxFrameStateClassifier(OnnxModelLoader loader,
                                    String modelKey,
                                    int width,
                                    int height,
                                    float[] mean,
    ...
    public Map<String, Object> predict(BufferedImage img) {
        try {
            ...
            try (OnnxTensor tensor = OnnxTensor.createTensor(env, FloatBuffer.wrap(input), shape);
                ...
                Map<FrameState, Double> probs = softmax(logits);
                Map<String, Object> out = new LinkedHashMap<>();
                Map<String, Double> asStrings = new LinkedHashMap<>();
                probs.forEach((k, v) -> asStrings.put(k.getValue(), v));
                out.put("frame_state_probabilities", asStrings);
                return out;
            }
        } catch (Exception ex) {
            throw new RuntimeException("Frame state ONNX inference failed", ex);
        }
    }

    private Map<FrameState, Double> softmax(float[] logits) {
        ...
        Map<FrameState, Double> probs = new LinkedHashMap<>();
