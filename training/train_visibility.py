...
Uses ONLY labeled images where frameState == "good".
Exports to ONNX format for Java inference.

Usage:
    python train_visibility.py --bucket <bucket-name>
"""
...
def main():
    ...
    # Filter for visibility training (only good frames)
    ...
    if len(visibility_data) < 100:
        logger.warning(f"Warning: Only {len(visibility_data)} images found. Consider labeling more data.")
    ...
    # Get visibility label mapping
    visibility_labels = config.get_visibility_labels()
    label_to_idx = config.get_visibility_label_to_index()
    num_classes = len(visibility_labels)
    
    logger.info(f"Visibility classes: {visibility_labels}")
    ...
    for epoch in range(config.training_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            best_epoch = epoch + 1
            checkpoint_path = config.output_checkpoint_dir / 'best_visibility_model.pt'
            torch.save({
                'epoch': epoch,
    ...
    onnx_filename = f"visibility_v{config.training_visibility_model_version}.onnx"
    onnx_path = config.output_models_dir / f"v{config.training_visibility_model_version}" / onnx_filename
    ...
    if args.upload:
        bucket_name = bucket
        s3_prefix = config.get_s3_model_version_prefix(config.training_visibility_model_version)
        logger.info(f"Uploading visibility model to s3://{bucket_name}/{s3_prefix}")
        uploader = S3ModelUploader(bucket_name, config.aws_region)
        uploader.upload_model(
            onnx_path,
            metadata_path,
            s3_prefix,
            config.training_visibility_model_version,
            model_filename=onnx_filename,
            metadata_filename=metadata_path.name
        )
        logger.info("Visibility model upload complete")
    else:
