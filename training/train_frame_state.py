...
Usage:
    ...
    python train_frame_state.py --version 2
"""
...
FRAME_STATE_LABELS = ['good', 'dark', 'bad', 'off_target']
...
def load_class_weights(version: int, models_dir: Path, device) -> torch.Tensor:
    ...
    weights_dict = analysis.get('frame_state', {}).get('weights', {})
    if not weights_dict:
        return None
    
    weights = [weights_dict.get(label, 1.0) for label in FRAME_STATE_LABELS]
    logger.info(f"Loaded class weights: {dict(zip(FRAME_STATE_LABELS, weights))}")
...
def log_confusion_matrix(matrix, labels):
    ...
    for i, row_label in enumerate(labels):
        ...
        logger.info(f"{row_label:>10} {row_str}")
    ...
    for i, label in enumerate(labels):
        ...
        logger.info(f"  {label:12} P={precision:.3f} R={recall:.3f} F1={f1:.3f}")
...
def filter_frame_state_labels(labels):
    """Filter labels for frame state training."""
    return [label for label in labels if label.frame_state and not label.excluded]
...
def main():
    ...
    dist = Counter(l.frame_state for l in frame_state_data)
    ...
    label_to_idx = {l: i for i, l in enumerate(FRAME_STATE_LABELS)}
    ...
    train_dataset = TrainingDataset(
        ...
        transform=train_transform, prefetch=False, label_field='frame_state'
    )
    val_dataset = TrainingDataset(
        labels=val_labels, image_loader=image_loader, label_to_index=label_to_idx,
        transform=val_transform, prefetch=False, label_field='frame_state'
    )
    ...
    checkpoint_path = output_dir / 'best_frame_state_model.pt'
    ...
    per_class_metrics = log_confusion_matrix(conf_matrix, FRAME_STATE_LABELS)
    ...
    metrics_path = output_dir / 'frame_state_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    logger.info(f"Saved metrics to {metrics_path}")

    # Export to ONNX
    onnx_filename = f"frame_state_v{version}.onnx"
